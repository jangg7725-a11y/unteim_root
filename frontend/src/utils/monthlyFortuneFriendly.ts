...
export function buildMonthlyFriendlyParagraphs(m: MonthlyFortuneEngineMonth): string[] {
  ...
  const monthlyPaceLine: Record<number, string> = {
    ...
    3: "3월은 속도가 붙는 달이라 중간 점검 시간을 반드시 넣어야 합니다.",
  ...
  if (!hasSeun && !hasDae) {
    out.push(
      `${month}월은 연간 흐름보다 당장 눈앞의 선택이 결과를 더 많이 좌우합니다. 핵심은 **${focusLine}**입니다. ${monthDecisionLine[month]}`
    );
  } else {
    out.push(
      `${month}월은 단기 흐름과 장기 흐름이 함께 움직입니다. 그래서 반응보다 **${focusLine}** 같은 운영이 더 중요합니다. ${monthDecisionLine[month]}`
    );
  }
  ...
  if (hasGongmang) {
    out.push(
      `${month}월은 계획이 한 번에 확정되지 않고 수정될 수 있어요. ${monthDecisionLine[month]}`
    );
  }
