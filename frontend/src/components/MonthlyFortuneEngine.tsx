...
export function MonthlyFortuneEngine({ data, onGoCounsel }: Props) {
  ...
  return (
    <section className="mfb" aria-labelledby="mfb-title">
      <div className="mfb__intro">
        <h2 id="mfb-title" className="mfb__title">
          월별 운세 리포트 ({data.year}년)
        </h2>
          ...
          <div className="mfb__book-inner">
            ...
            <dl className="mfb__signals">
              <div>
                <dt>세운 · 대운</dt>
                <dd>
                  세운 {m.seunPillar || "—"} / 대운 {m.daewoonPillar || "—"}
                </dd>
              </div>
              <div>
                <dt>지지 합·충</dt>
                <dd>
                  {m.interactionHints.length > 0
                    ? m.interactionHints.join(" ")
                    : "원국과 두드러진 합·충 신호는 약한 편으로 읽을 수 있습니다."}
                </dd>
              </div>
              <div>
                <dt>공망</dt>
                <dd>{m.gongmangLine || "—"}</dd>
              </div>
              <div>
                <dt>용신 · 희신 · 기신 반응</dt>
                <dd>{m.yongshinLine || "—"}</dd>
              </div>
              {m.patternTop.length > 0 && (
                <div>
                  <dt>월운 패턴</dt>
                  <dd>{m.patternTop.join(" · ")}</dd>
                </div>
              )}
            </dl>
