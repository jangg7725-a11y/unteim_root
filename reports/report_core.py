...
# ✅ [v2] 자기이해 문장 엔진
try:
    from engine.sentences_v2_engine import (
        attach_v2_sentences,
        get_texts,
    )
    _HAS_V2 = True
...
def _build_pdf_report(
    ...
    for fn, sec_name in [
        ...
        try:
            ...
            story.append(Spacer(1, 6))
        # ✅ [v2] 자기이해 문장 + 실천 개운법 섹션
    if _HAS_V2:
        try:
            if not isinstance(report.get("v2_sentences"), dict):
                attach_v2_sentences(report)

            v2 = report.get("v2_sentences", {})
            if isinstance(v2, dict) and "error" not in v2:
                story.append(PageBreak())

                body = styles.get("KBody") or next(iter(styles.values()))
                h2 = styles.get("KH2") or styles.get("KSubtitle") or body
                h3 = styles.get("KH3") or h2

                story.append(Paragraph("🧭 자기이해 리포트", h2))
                story.append(Spacer(1, 8))
                story.append(Paragraph(
                    "이 리포트는 운명을 규정하지 않습니다. 자신의 패턴을 이해하고, 더 밝은 방향으로 나아갈 수 있도록 돕는 나침반입니다.",
                    body
                ))
                story.append(Spacer(1, 12))
                ...
                def append_why_pattern_action_section(section_title, texts):
                    """
                    WHY / PATTERN / ACTION 분류 후 A-1과 동일 순서로 출력.
                    (표시 순서: PATTERN → WHY → ACTION, 내부 역할명과 동일)
                    """
                    if not texts:
                        return
                    ...
                    # PATTERN
                    if pattern:
                        story.append(Paragraph("✔ 핵심 특징", h3))
                        for t in pattern[:3]:
                    ...
                    # WHY
                    if why:
                        story.append(Paragraph("✔ 왜 이런 흐름인가", h3))
                        for t in why[:2]:
                            story.append(Paragraph(f"• {t}", body))
                            story.append(Spacer(1, 3))
                        story.append(Spacer(1, 6))
                    ...
                    # ACTION
                    if action:
                        story.append(Paragraph("✔ 이렇게 하세요", h3))
                        for t in action[:2]:
                            story.append(Paragraph(f"• {t}", body))
                            story.append(Spacer(1, 3))
                ...
                v2m = v2.get("meta", {}) or {}
                append_why_pattern_action_section(
                    "🌿 오행으로 보는 당신의 성격 패턴",
                    get_texts(v2.get("A1_oheng_personality", [])),
                )
                append_why_pattern_action_section(
                    f"👤 일간({v2m.get('day_gan', '')}) 프로필",
                    get_texts(v2.get("A2_day_master", [])),
                )
                append_why_pattern_action_section(
                    "🌟 신살 & 12운성 해석",
                    get_texts(v2.get("A3_shinsal_fortunes", [])),
                )
                ...
                if b1:
                    story.append(PageBreak())
                    story.append(Paragraph("🛠️ 실천 개운법", h2))
                    story.append(Spacer(1, 6))
                    story.append(Paragraph(f"당신의 용신 오행: <b>{b1.get('오행', '')}</b>", h3))
                    story.append(Spacer(1, 4))

                    tbl_data = [["항목", "내용"]]
                    for k in ["색상", "숫자", "방위", "계절", "맛", "대표음식", "보석/소재", "추천 활동"]:
                        if b1.get(k):
                            tbl_data.append([k, b1[k]])

                    tbl = Table(tbl_data, colWidths=[80, 380])
                    tbl.setStyle(TableStyle([
                        ("BACKGROUND", (0, 0), (-1, 0), colors.Color(0.78, 0.66, 0.30)),
                        ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
                        ("FONTNAME", (0, 0), (-1, -1), "NotoSansKR"),
                        ("FONTSIZE", (0, 0), (-1, -1), 9),
                        ("GRID", (0, 0), (-1, -1), 0.5, colors.Color(0.83, 0.79, 0.72)),
                        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                        ("BACKGROUND", (0, 1), (0, -1), colors.Color(0.96, 0.94, 0.89)),
                    ]))
                    story.append(tbl)
                    story.append(Spacer(1, 10)) 
                # B-2 용신별 실천 조언
                b2 = get_texts(v2.get("B2_yongshin_advice", []))
                if b2:
                    story.append(Paragraph("✨ 오늘부터 할 수 있는 실천 개운법", h3))
                    story.append(Spacer(1, 4))
                    for i, t in enumerate(b2, 1):
                        story.append(Paragraph(f"{i}. {t}", body))
                        story.append(Spacer(1, 3))
                    story.append(Spacer(1, 8))

                # B-5 삼재
                b5 = get_texts(v2.get("B5_samjae", []))
                if b5:
                    story.append(Paragraph("🌬️ 삼재(三災) 해석", h2))
                    story.append(Spacer(1, 4))
                    for t in b5[:3]:
                        story.append(Paragraph(f"• {t}", body))
                        story.append(Spacer(1, 3))
                ...
                # C 마무리
                c3 = get_texts(v2.get("C_closing", []))
                c4 = get_texts(v2.get("C_affirmation", []))
                if c3 or c4:
                    story.append(PageBreak())
                    story.append(Paragraph("💌 마무리", h2))
                    story.append(Spacer(1, 6))
                    import random
                    selected = random.sample(c3 + c4, min(5, len(c3 + c4)))
                    for t in selected:
                        story.append(Paragraph(f"• {t}", body))
                        story.append(Spacer(1, 4))
                    story.append(Spacer(1, 12))
                    story.append(Paragraph(
                        "당신의 기운을 이해하는 것이 가장 큰 개운의 시작입니다. 🌟", body))

        except Exception as e:
            story.append(Paragraph(
                f"[WARN] v2 section skipped: {type(e).__name__}: {e}",
                styles.get("KSmall") or next(iter(styles.values()))))
            story.append(Spacer(1, 6))
